...
public class RulesContentProvider extends ContentProvider {
	...
	public synchronized String getType(Uri uri) {
		...
		String contentType = RulesContract.CONTENT_ITEM_TYPE;

		if (isTableUri(uri)) {

			contentType = RulesContract.CONTENT_DIR_TYPE;
	...
	public synchronized Uri insert(Uri uri, ContentValues value) {
		...
		if (value.containsKey(RulesContract.DATA)) {

			DataRecord dataRecord = new DataRecord(value.getAsString(RulesContract.DATA));
			db.put(dataRecord.getID(), dataRecord);

			// return Uri associated with newly-added data item
			return Uri.withAppendedPath(RulesContract.CONTENT_URI,
	...
	public synchronized Cursor query(Uri uri, String[] projection,
		...
		MatrixCursor cursor = new MatrixCursor(RulesContract.ALL_COLUMNS);
	...
	private boolean isTableUri(Uri uri) {
		return uri.getLastPathSegment().equals(RulesContract.DATA_TABLE);
	}
