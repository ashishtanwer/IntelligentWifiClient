...
public class LocationContentProvider extends ContentProvider {
	...
	private static final String TAG = "RulesContentProvider";
	...
	public synchronized String getType(Uri uri) {
		...
		String contentType = LocationContract.CONTENT_ITEM_TYPE;

		if (isTableUri(uri)) {

			contentType = LocationContract.CONTENT_DIR_TYPE;
	...
	public synchronized Uri insert(Uri uri, ContentValues value) {
		...
		if (value.containsKey(LocationContract.DATA)) {

			DataRecord dataRecord = new DataRecord(value.getAsString(LocationContract.DATA));
			db.put(dataRecord.getID(), dataRecord);

			// return Uri associated with newly-added data item
			return Uri.withAppendedPath(LocationContract.CONTENT_URI,
	...
	public synchronized Cursor query(Uri uri, String[] projection,
		...
		MatrixCursor cursor = new MatrixCursor(LocationContract.ALL_COLUMNS);
	...
	private boolean isTableUri(Uri uri) {
		return uri.getLastPathSegment().equals(LocationContract.DATA_TABLE);
	}
